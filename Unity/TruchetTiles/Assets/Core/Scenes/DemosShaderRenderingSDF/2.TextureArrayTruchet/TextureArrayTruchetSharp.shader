Shader "Custom/TextureArrayTruchetSharp"
{
    Properties
    {
        ...
        // Brought thickness back to the shader!
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                // Determine Shape Index (0: Arcs, 1: Cross)
                uint texIndex = (rndShape < 0.6) ? 0 : 1;

                // 1. Sample the raw mathematical distance (the "Skeleton") from the EXR
                float centerDist = SAMPLE_TEXTURE2D_ARRAY(_ShapeMapArray, sampler_ShapeMapArray, localUV, texIndex).r;

                // 2. Expand the skeleton by subtracting half the thickness
                // Values < 0 mean we are INSIDE the line. Values > 0 mean we are OUTSIDE.
                float finalSDF = centerDist - (_Thickness * 0.5);

                // 3. Rasterization: Convert the SDF gradient into a razor-sharp, anti-aliased mask
                float pixelSize = fwidth(finalSDF);
                float shapeMask = 1.0 - smoothstep(0.0, pixelSize * 1.5, finalSDF);
